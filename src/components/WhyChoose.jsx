...
return ( <div className="py-20 bg-white text-center">
{/* Section Title */} <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-black !text-black mb-12 px-4">
Experience Journeys Crafted With Heart & Heritage </h2>
  ...
  >
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 border border-[var(--gold-dark)]"
      >
        <h3 className="text-lg sm:text-xl font-semibold text-black !text-black mb-4">
          {features[index].title}
        </h3>
        <p className="text-black !text-black leading-relaxed text-sm sm:text-base">
      ...
      {features.map((_, i) => (
        <div
          ...
          className={`h-1 rounded-full transition-all duration-500 ${
            i === index ? "w-10 bg-[var(--gold-dark)]" : "w-6 bg-gray-400"
          }`}
        ></div>
      ))}
    </div>
